...
void AShooterWeapon::StartReload(bool bFromReplication)
{
	...
	{		
		...
		{
			// Wwise Reload
			FAkAudioDevice::Get()->PostEvent(assault_rifle_reload, this);
...
void AShooterWeapon::HandleFiring()
{
	...
	else if (MyPawn && MyPawn->IsLocallyControlled())
	{
		...
		{
			// Wwise Out Of Ammo
			FAkAudioDevice::Get()->PostEvent(assault_rifle_out_of_ammo, this);
...
void AShooterWeapon::SimulateWeaponFire()
{
	...
	{
		// Wwise Fire Loop
		FAkAudioDevice::Get()->PostEvent(assault_rifle_fire_loop, this);
	...
	// Wwise RTPC Rifle
	FAkAudioDevice::Get()->SetRTPCValue(*FString("RTPC_ammo_HPF"), CurrentAmmoInClip, 0, NULL);
	FAkAudioDevice::Get()->SetRTPCValue(*FString("RTPC_ammo_Pitch"), CurrentAmmoInClip, 0, NULL);
}
...
void AShooterWeapon::StopSimulatingWeaponFire()
{
	...
	if (bLoopedFireAnim && bPlayingFireAnim)
	{
		// Wwise Fire Tail and Single Shot
		FAkAudioDevice::Get()->PostEvent(assault_rifle_fire_tail, this);
